...
assert(vim.fn.hlexists('StringDelimiter') == 1, 'missing StringDelimiter highlight')
assert(vim.fn.hlexists('@string.delimiter') == 1, 'missing @string.delimiter highlight')
...
local dl = vim.api.nvim_get_hl(0, { name = 'Delimiter', link = false })
local sdl = vim.api.nvim_get_hl(0, { name = 'StringDelimiter', link = false })
local tsdl = vim.api.nvim_get_hl(0, { name = '@string.delimiter', link = false })
...
assert(dl.fg == sdl.fg, 'StringDelimiter should match Delimiter')
assert(dl.fg == tsdl.fg, '@string.delimiter should match Delimiter')
