...
assert(vim.fn.hlexists('@keyword.function.python') == 1, 'missing @keyword.function.python highlight')
assert(vim.fn.hlexists('@keyword.function.javascript') == 1, 'missing @keyword.function.javascript highlight')
assert(vim.fn.hlexists('@keyword.function.typescript') == 1, 'missing @keyword.function.typescript highlight')
...
local kfp = vim.api.nvim_get_hl(0, { name = '@keyword.function.python', link = false })
local kfj = vim.api.nvim_get_hl(0, { name = '@keyword.function.javascript', link = false })
local kft = vim.api.nvim_get_hl(0, { name = '@keyword.function.typescript', link = false })
...
if '${style}' == 'flared' then
  assert(kfp.fg == ks.fg, '@keyword.function.python should match @keyword.storage in flared')
  assert(kfj.fg == ks.fg, '@keyword.function.javascript should match @keyword.storage in flared')
  assert(kft.fg == ks.fg, '@keyword.function.typescript should match @keyword.storage in flared')
